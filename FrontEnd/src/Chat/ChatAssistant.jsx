...
const ChatAssistant = ({ userId, hash, token }) => {
	const [messages, setMessages] = useState([
		{ role: "assistant", content: "Hi! Ask me about your tasks or say 'summarize'." },
	]);
	...
	return (
		<div className="card p-4 flex flex-col h-full">
			<div ref={listRef} className="flex-1 overflow-y-auto space-y-3 pr-1">
				{messages.map((m, idx) => (
					<div key={idx} className={m.role === "assistant" ? "chat-bubble ai" : "chat-bubble user"}>
						{m.content}
					</div>
				))}
			</div>
